<?php
  ...
  session_start();
   ...
   function readprofilecolors(){
    ...
    $stmt = $mysqli->prepare("SELECT bgcolor, txtcolor FROM vpuserprofiles WHERE user_id=?");
	echo $mysqli->error;
  ...
  function validatemsg($editId, $validation){
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $mysqli->prepare("UPDATE vpamsg SET acepptedby=?, accepted=?, accepttime=now() WHERE id=?");
